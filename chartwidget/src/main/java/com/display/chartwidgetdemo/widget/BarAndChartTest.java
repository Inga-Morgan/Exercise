package com.display.chartwidgetdemo.widget;
...
import com.display.chartwidgetdemo.utils.DensityUtil;
...
public class BarAndChartTest extends View {
    ...
    private BarAndChartTest.OnItemBarClickListener mOnItemBarClickListener;
    ...
    private List<List<Integer>> mRightDatas;
    ...
    public void setOnItemBarClickListener(BarAndChartTest.OnItemBarClickListener onRangeBarClickListener) {
    ...
    public BarAndChartTest(Context context) {
        super(context);
        init(context);
    }

    public BarAndChartTest(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public BarAndChartTest(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        mGestureListener = new GestureDetector(context, new BarAndChartTest.RangeBarOnGestureListener());
...
//        mLinePaint.setPathEffect(new DashPathEffect(new float[]{15, 5}, 0));
    ...
    public void setData(List<BarChartEntity> list, int[] colors, String mUnitX, String mUnitY, List<List<Integer>> rightDatas, int textType) {
     ...
     */
    public void setData(List<BarChartEntity> list, int[] colors, String mUnitX, String mUnitY, List<List<Integer>> rightDatas, int textType, int[] rightScale) {
        this.mData = list;
        ...
        if (mRightDatas != null) {
            mRightPaths.clear();
            for (List<Integer> rightData : mRightDatas) {
    ...
    private float getRightMaxValue(int type) {
        ...
        for (Integer tem : mRightDatas.get(0)) {
            if (tem > maxValue) {
                maxValue = tem;
            }
            if (tem < minValue) {
...
//            }
    ...
    private void drawPath(int position) {
        for (int i = 0; i < mRightPaths.size(); i++) {
            ...
            List<Integer> rightData = mRightDatas.get(i);
    ...
    private void drawBar(Canvas canvas) {
        ...
        for (int i = 0; i < mData.size(); i++) {
            if (mBarColors.length == 1) {
                ...
                mBarRect.top = (int) (mStartY - (int) ((maxHeight * (mData.get(i).getyValue() / mMaxYDivisionValue)) * percent));
                ...
                for (int j = 0; j < mBarColors.length; j++) {
                    ...
                    eachHeight += (int) ((maxHeight * (mData.get(i).getyValue() / mMaxYDivisionValue)));
                    mBarRect.top = (int) (mBarRect.bottom - ((maxHeight * (mData.get(i).getyValue() / mMaxYDivisionValue))) * percent);
                    canvas.drawRect(mBarRect, mBarPaint);
                }
            }
    ...
    private void drawBorder(int position) {
        ...
        mBarRectClick.top = (int) (mStartY - (int) (maxHeight * (mData.get(position).getyValue() / mMaxYDivisionValue)));
    }
